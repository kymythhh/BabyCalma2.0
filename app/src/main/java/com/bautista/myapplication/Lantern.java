...
public class Lantern extends AppCompatActivity {
    ...
    private void addLantern() {
        ...
        // Save to database
        final String stressorTextFinal = text;
        repository.saveLantern(stressorTextFinal, currentDate,
                new BabyCalmaRepository.DataCallback<Long>() {
                    @Override
                    public void onDataLoaded(Long lanternId) {
                        // Store mapping between view and database ID
    ...
    private void releaseLantern(final View lanternView) {
        ...
        // Get database ID and update database
        Integer lanternId = lanternViewToIdMap.get(lanternView);
        if (lanternId != null) {
            repository.releaseLantern(lanternId,
                    new BabyCalmaRepository.DatabaseCallback() {
                        @Override
                        public void onSuccess() {
                            // Successfully released in database
    ...
    private void updateStats() {
        // Deprecated - use loadStatsFromDatabase() instead
        emptyStateView.setVisibility(lanterns.isEmpty() ? View.VISIBLE : View.GONE);
    }
    ...
    private void loadStatsFromDatabase() {
        repository.getLanternStats(currentDate,
                new BabyCalmaRepository.DataCallback<BabyCalmaRepository.LanternStats>() {
                    @Override
                    public void onDataLoaded(BabyCalmaRepository.LanternStats stats) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                totalCountText.setText(String.valueOf(stats.total));
                                activeCountText.setText(String.valueOf(stats.active));
                                releasedCountText.setText(String.valueOf(stats.released));
    ...
    /**
     * Load active lanterns from database
     */
    private void loadActiveLanterns() {
        repository.getActiveLanterns(currentDate,
                new BabyCalmaRepository.DataCallback<List<LanternEntity>>() {
                    @Override
                    public void onDataLoaded(List<LanternEntity> lanternEntities) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                // Clear existing views
                                lanterns.clear();
                                lanternContainer.removeAllViews();
                                lanternViewToIdMap.clear();

                                // Recreate lanterns from database
                                for (LanternEntity entity : lanternEntities) {
    ...
    /**
     * Recreate a lantern view from database entity
     */
