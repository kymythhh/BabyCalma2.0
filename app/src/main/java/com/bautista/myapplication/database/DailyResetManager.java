...
public class DailyResetManager {
     ...
     */
    public void checkAndResetIfNeeded(final ResetCallback callback) {
        executor.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (!currentDate.equals(lastResetDate)) {
                    ...
                    performDailyReset(lastResetDate, currentDate);
     ...
     */
    private void performDailyReset(String lastDate, String currentDate) {
        try {
            // Save yesterday's stats before resetting
            if (!lastDate.isEmpty()) {
                saveDailyStats(lastDate);
            }
            
            // Reset today's data (delete current day's data)
            resetTodayData(currentDate);
     ...
     */
    private void saveDailyStats(String date) {
        ...
        int totalWater = waterDao.getTotalGlassesForDate(date);
        int totalBreathingCycles = breathingDao.getTotalCyclesForDate(date);
        int totalFocusMinutes = focusDao.getTotalMinutesForDate(date);
        int totalLanternsReleased = lanternDao.getReleasedCountForDate(date);
        ...
        DailyStatsEntity existingStats = statsDao.getStatsForDate(date);
     ...
     */
    private void resetTodayData(String currentDate) {
        // Delete today's entries to start fresh
        database.waterIntakeDao().deleteByDate(currentDate);
        database.breathingSessionDao().deleteByDate(currentDate);
        database.focusSessionDao().deleteByDate(currentDate);
        database.lanternDao().deleteByDate(currentDate);
    }
    
    /**
     * Clean up old data (keep only last 30 days)
     */
    private void cleanupOldData(String currentDate) {
        try {
            ...
            if (current != null) {
                ...
                // Delete old records
                database.waterIntakeDao().deleteOlderThan(cutoffDate);
                database.breathingSessionDao().deleteOlderThan(cutoffDate);
                database.focusSessionDao().deleteOlderThan(cutoffDate);
                database.lanternDao().deleteOlderThan(cutoffDate);
                database.dailyStatsDao().deleteOlderThan(cutoffDate);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void forceReset() {
        executor.execute(new Runnable() {
            ...
            public void run() {
                String currentDate = getCurrentDate();
                performDailyReset(currentDate, currentDate);
                prefs.edit().putString(KEY_LAST_RESET_DATE, currentDate).apply();
            }
        });
    }
